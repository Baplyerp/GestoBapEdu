...
# ==========================================
    ...
    # ==========================================
        ...
        # --- 🔄 MÓDULO 2: CICLO DE FOCO (NEUROCIÊNCIA) ---
        with tab_ciclo:
            import time
            from datetime import timedelta

            col_roleta, col_timer = st.columns([1, 1.2])

            with col_roleta:
                st.markdown("#### 🎡 Sua Roleta de Matérias")
                
                with Session(get_engine()) as session:
                    todas_disciplinas = session.query(Disciplina).all()
                    opcoes_disc = {d.nome: d.id for d in todas_disciplinas}

                # Configuração do Ciclo no Session State para não perder ao clicar
                if "ciclo_ordem" not in st.session_state:
                    st.session_state.ciclo_ordem = []
                
                ciclo_input = st.multiselect(
                    "Defina a sequência do seu ciclo:",
                    options=list(opcoes_disc.keys()),
                    default=st.session_state.ciclo_ordem,
                    help="A neurociência sugere alternar matérias de naturezas diferentes (ex: Exatas -> Direito)."
                )
                st.session_state.ciclo_ordem = ciclo_input

                if not st.session_state.ciclo_ordem:
                    st.warning("Adicione matérias para montar sua roleta de estudo.")
                else:
                    # Mostrar visualmente o próximo passo
                    st.markdown("---")
                    for i, materia in enumerate(st.session_state.ciclo_ordem):
                        seta = "➡️" if i == 0 else "▫️"
                        st.markdown(f"{seta} **{materia}**")
                    st.caption("O ciclo reinicia automaticamente ao chegar no fim.")

            with col_timer:
                st.markdown("#### ⏱️ Timer Pomodoro de Elite")
                
                if not st.session_state.ciclo_ordem:
                    st.info("Aguardando definição do ciclo...")
                else:
                    materia_atual = st.session_state.ciclo_ordem[0]
                    st.success(f"Foco Atual: **{materia_atual}**")
                    
                    # Estado do Timer
                    if "timer_rodando" not in st.session_state: st.session_state.timer_rodando = False
                    if "tempo_inicio" not in st.session_state: st.session_state.tempo_inicio = None

                    c_t1, c_t2 = st.columns(2)
                    
                    if not st.session_state.timer_rodando:
                        if c_t1.button("▶️ Iniciar Sessão (50 min)", use_container_width=True, type="primary"):
                            st.session_state.timer_rodando = True
                            st.session_state.tempo_inicio = time.time()
                            st.rerun()
                    else:
                        if c_t1.button("⏹️ Parar e Registrar", use_container_width=True):
                            # Cálculo de tempo gasto
                            tempo_final = time.time()
                            segundos_gastos = int(tempo_final - st.session_state.tempo_inicio)
                            
                            # Registro no Banco
                            with Session(get_engine()) as session:
                                try:
                                    id_disc = opcoes_disc[materia_atual]
                                    nova_sessao = SessaoEstudo(
                                        user_id=uuid.UUID(st.session_state.utilizador.id),
                                        disciplina_id=id_disc,
                                        duracao_segundos=segundos_gastos,
                                        foco_score=4 # Padrão, depois podemos por um slider
                                    )
                                    session.add(nova_sessao)
                                    session.commit()
                                    
                                    # Gira a roleta (move a primeira matéria para o fim)
                                    nova_ordem = st.session_state.ciclo_ordem[1:] + [st.session_state.ciclo_ordem[0]]
                                    st.session_state.ciclo_ordem = nova_ordem
                                    
                                    st.session_state.timer_rodando = False
                                    st.success(f"✅ {segundos_gastos // 60} min registrados em {materia_atual}!")
                                    st.balloons()
                                    time.sleep(2)
                                    st.rerun()
                                except Exception as e:
                        ...
                        # Visualização do tempo (Placeholder simples para manter performance)
                        if st.session_state.timer_rodando:
                            elapsed = int(time.time() - st.session_state.tempo_inicio)
                            minutos = elapsed // 60
                            segundos = elapsed % 60
                            st.metric("Tempo em Foco", f"{minutos:02d}:{segundos:02d}")
                            
                            # Botão de refresh manual (Streamlit limita auto-refresh de segundos)
                            if st.button("🔄 Atualizar Cronômetro"):
                                st.rerun()
                                
                    st.markdown("""
                        <div style='background-color: #FDFEFE; padding: 15px; border-radius: 10px; border: 1px dashed #D4AF37; margin-top: 20px;'>
                            <small><b>Dica de Neurociência:</b> Após o timer, faça uma pausa de 10 min longe de telas para consolidar a memória.</small>
                        </div>
                    """, unsafe_allow_html=True)
